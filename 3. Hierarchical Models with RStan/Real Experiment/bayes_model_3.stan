...
model {
	...
	beta_mu ~ normal(0,1);
	beta_sd ~ cauchy(0,3);
	// "откуда" (из какого распределения по популяции) генерировались индивидуальные размеры эффекта и индивидуальные смещения
	...
	for(i in 1:nY){
		Y[i] ~ bernoulli_logit(intercept[Subj[i]] + beta[Subj[i]]*size_diff[i]);
	}
